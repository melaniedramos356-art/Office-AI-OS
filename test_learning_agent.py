...
def test_learning_agent_directly():
    ...
    production_path = memory_folder / "production_techniques.md"
    ...
    if not production_path.exists():
        raise AssertionError(f"制作技巧文件不存在：{production_path}")
    ...
    if "word 制作技巧" not in production_content:
        raise AssertionError("制作技巧文件没有包含 Word 制作技巧。")

    if "可参考素材结构" not in production_content:
        raise AssertionError("制作技巧文件没有写入从素材学习到的结构。")
