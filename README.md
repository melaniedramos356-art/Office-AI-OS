...
```text
第一优先模型可用但调用失败：自动尝试下一个可用模型
所有真实模型都失败：自动回到本地规则生成
普通测试默认使用模拟模型，不消耗真实 API 调用
真实 API 连通性单独用 test_deepseek_client.py 或 test_openai_client.py 测试
```

提示词压缩：

```text
模型调用前会自动清理重复行、合并多余空行
超长提示词会保留开头需求和结尾约束，中间内容压缩
默认最大长度为 6000 字符，可用 OFFICE_AI_MAX_PROMPT_CHARS 调整
测试命令：python test_prompt_optimizer.py
```
...
```bash
python test_kimi_real_client.py
```
...
```text
KIMI_API_KEY：项目内推荐使用的 Kimi API Key 环境变量
MOONSHOT_API_KEY：Kimi 官方文档推荐的 API Key 环境变量，程序也兼容
KIMI_MODEL：Kimi 模型名，不设置时默认 kimi-k2.6
...
DeepSeek / Kimi：通过 Model Router 提供不同 AI 能力
